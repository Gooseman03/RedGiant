...
public class CarbonScrubber : MonoBehaviour
{
    ...
    {
        if (baseSystem.itemRegister.HasObject(ObjectType.Co2Canister, out List<ObjectGrabbable> ObjectList))
        {
            int i = 0;
            foreach (ObjectGrabbable Co2Canister in ObjectList)
            {
    ...
    {
        if (!ErrorCodes.CheckWorking(baseSystem.itemRegister, ObjectType.PowerConnector))
        {
            return false;
        }
        if (!ErrorCodes.CheckWorking(baseSystem.itemRegister, ObjectType.Fuse))
        {
            return false;
        }
        return true;
